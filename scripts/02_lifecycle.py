...
datasets -- see /DATASET_FORMAT.md. Reads <dataset>/outputs/fmeca_worksheet.csv
etc (written by 01_pipeline.py) and applies the same decision logic to whatever
groups that dataset defines.

All assumption parameters (repair complexity, lead time, unit cost, fleet size)
come from the dataset's config.yaml `assumptions:` block -- author-defined
illustrative values, clearly separated from the real O/S/D numbers computed
...
# ---------------------------------------------------------------------------
# assumption parameters (from config.yaml, NOT from the dataset itself)
# ---------------------------------------------------------------------------
A = cfg.assumptions
ASSUMED_LEAD_DAYS = A["lead_days"]
ASSUMED_UNIT_COST_KRW = A["unit_cost_krw"]
ASSUMED_REPAIR_COMPLEXITY = A["repair_complexity"]
ASSUMED_FLEET_SIZE = A["fleet_size"]
ASSUMED_DAILY_OP_TIMESTEPS = A["daily_op_timesteps"]
Z_SERVICE_95 = 1.645

rows = []
for _, r in ws.iterrows():
    grp = r.failure_mode_group
    label = f"{cfg.group_label(grp)}({cfg.group_col}={grp})"
    ...
    fleet_n_this_group = ASSUMED_FLEET_SIZE / n_groups  # 그룹 수만큼 균등 배분 가정
    ...
    if significant and pd.notna(detr.mean_lead_time):
        ...
        ietm_proc = f"IETM-{CODE}-{grp}-01 (이상신호 대응 점검절차)"
    else:
        sla = None
        ietm_note = "유의한 사전 이상신호 없음 — 사전점검 절차 정의 근거 부족, 정기점검 주기 강화로 대체"
        ietm_proc = f"IETM-{CODE}-{grp}-02 (정기점검 강화 절차)"
...
with open(f"{OUT}/lifecycle.json", "w", encoding="utf-8") as f:
    json.dump(dict(
        assumptions=dict(lead_days=ASSUMED_LEAD_DAYS, unit_cost_krw=ASSUMED_UNIT_COST_KRW,
                          ...
                          service_level="95%(z=1.645)", note=A.get("note", "보급소요 파라미터는 전량 가정치 -- 실측이 아님.")),
        rows=rows,
